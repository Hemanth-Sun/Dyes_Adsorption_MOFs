...
OUTPUT_DIR = r"/users/hemanthsunkara/downloads/outs/XGB/MO"
...
xgb_random = RandomizedSearchCV(
    ...
    xgb_param_grid, n_iter=30, cv=5, scoring='r2',
...
cv_rmse = cross_val_score(best_xgb, X_train_scaled, y_train, cv=5, scoring='neg_root_mean_squared_error')
cv_r2 = cross_val_score(best_xgb, X_train_scaled, y_train, cv=5, scoring='r2')
print(f"\nXGBoost CV R²  : {cv_r2}")
print(f"XGBoost CV RMSE: {np.abs(cv_rmse)}")
print(f"XGBoost Mean CV RMSE: {np.mean(np.abs(cv_rmse)):.3f}")

# --- Step 12: Fit XGBoost Model ---
...
plt.title("XGBoost: Adsorption Capacity Prediction", fontsize=14, weight='bold')
...
plt.title("SHAP Summary Plot - XGBoost", fontsize=13, weight='bold')
plt.savefig(os.path.join(OUTPUT_DIR, "XGBoost_SHAP_summary.png"), dpi=300, bbox_inches="tight")
plt.close()

# SHAP bar plot
shap.summary_plot(shap_values_xgb, X_test_scaled, feature_names=X.columns,
                  plot_type="bar", show=False)
plt.title("SHAP Feature Importance - XGBoost", fontsize=13, weight='bold')
plt.savefig(os.path.join(OUTPUT_DIR, "XGBoost_SHAP_bar.png"), dpi=300, bbox_inches="tight")
plt.close()
...
output_path = os.path.join(OUTPUT_DIR, "XGBoost_Results.xlsx")
...
with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
    ...
    shap_xgb_df.to_excel(writer, sheet_name="SHAP_XGBoost", index=False)

print(f"\nAll results saved to '{output_path}'")
print("  Sheets: Actual_vs_Predicted | Actual_vs_Predicted_Train | Metrics | SHAP_XGBoost")
...
plot_rec_curve(y_test, y_pred_test, model_name="XGBoost", color="green")