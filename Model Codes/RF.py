...
OUTPUT_DIR = r"/users/hemanthsunkara/downloads/outs/rfres/MBMO"
...
rf_random = RandomizedSearchCV(
    ...
    rf_param_grid, n_iter=30, cv=5, scoring='r2',
    verbose=1, n_jobs=-1, random_state=42
)
rf_random.fit(X_train_scaled, y_train)
best_rf = rf_random.best_estimator_
print(f"Best RandomForest params: {rf_random.best_params_}")

# --- Step 11: Cross-Validation Scores ---
cv_rmse = cross_val_score(best_rf, X_train_scaled, y_train, cv=5, scoring='neg_root_mean_squared_error')
cv_r2 = cross_val_score(best_rf, X_train_scaled, y_train, cv=5, scoring='r2')
print(f"\nRandomForest CV R²  : {cv_r2}")
print(f"RandomForest CV RMSE: {np.abs(cv_rmse)}")
print(f"RandomForest Mean CV RMSE: {np.mean(np.abs(cv_rmse)):.3f}")

# --- Step 12: Fit RandomForest Model ---
...
plt.title("RandomForest: Adsorption Capacity Prediction", fontsize=14, weight='bold')
...
# --- Step 16: SHAP Analysis for RandomForest ---
...
plt.title("SHAP Summary Plot - RandomForest", fontsize=13, weight='bold')
plt.savefig(os.path.join(OUTPUT_DIR, "RandomForest_SHAP_summary.png"), dpi=300, bbox_inches="tight")
plt.close()

# SHAP bar plot
shap.summary_plot(shap_values_rf, X_test_scaled, feature_names=X.columns,
                  plot_type="bar", show=False)
plt.title("SHAP Feature Importance - RandomForest", fontsize=13, weight='bold')
plt.savefig(os.path.join(OUTPUT_DIR, "RandomForest_SHAP_bar.png"), dpi=300, bbox_inches="tight")
plt.close()
...
shap_rf_df = pd.DataFrame(shap_values_rf, columns=X.columns)
...
output_path = os.path.join(OUTPUT_DIR, "RandomForest_Results.xlsx")
...
with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
    ...
    shap_rf_df.to_excel(writer, sheet_name="SHAP_RandomForest", index=False)

print(f"\nAll results saved to '{output_path}'")
print("  Sheets: Actual_vs_Predicted | Actual_vs_Predicted_Train | Metrics | SHAP_RandomForest")
...
plot_rec_curve(y_test, y_pred_test, model_name="RandomForest", color="green")