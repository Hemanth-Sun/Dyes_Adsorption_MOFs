...
OUTPUT_DIR = r"/users/hemanthsunkara/downloads/outs/et/MO"
...
et_random = RandomizedSearchCV(
    ...
    et_param_grid, n_iter=30, cv=5, scoring='r2',
    verbose=1, n_jobs=-1, random_state=42
)
et_random.fit(X_train_scaled, y_train)
best_et = et_random.best_estimator_
print(f"Best ExtraTrees params: {et_random.best_params_}")

# --- Step 11: Cross-Validation Scores ---
cv_rmse = cross_val_score(best_et, X_train_scaled, y_train, cv=5, scoring='neg_root_mean_squared_error')
cv_r2 = cross_val_score(best_et, X_train_scaled, y_train, cv=5, scoring='r2')
print(f"\nExtraTrees CV R²  : {cv_r2}")
print(f"ExtraTrees CV RMSE: {np.abs(cv_rmse)}")
print(f"ExtraTrees Mean CV RMSE: {np.mean(np.abs(cv_rmse)):.3f}")

# --- Step 12: Fit ExtraTrees Model ---
best_et.fit(X_train_scaled, y_train)

# --- Step 13: Predictions ---
y_pred_test = best_et.predict(X_test_scaled)
y_pred_train = best_et.predict(X_train_scaled)
...
plt.title("ExtraTrees: Adsorption Capacity Prediction", fontsize=14, weight='bold')
...
# --- Step 16: SHAP Analysis for ExtraTrees ---
explainer_et = shap.TreeExplainer(best_et)
shap_values_et = explainer_et.shap_values(X_test_scaled)

# SHAP summary plot
shap.summary_plot(shap_values_et, X_test_scaled, feature_names=X.columns, show=False)
plt.title("SHAP Summary Plot - ExtraTrees", fontsize=13, weight='bold')
plt.savefig(os.path.join(OUTPUT_DIR, "ExtraTrees_SHAP_summary.png"), dpi=300, bbox_inches="tight")
plt.close()

# SHAP bar plot
shap.summary_plot(shap_values_et, X_test_scaled, feature_names=X.columns,
                  plot_type="bar", show=False)
plt.title("SHAP Feature Importance - ExtraTrees", fontsize=13, weight='bold')
plt.savefig(os.path.join(OUTPUT_DIR, "ExtraTrees_SHAP_bar.png"), dpi=300, bbox_inches="tight")
plt.close()
...
shap_et_df = pd.DataFrame(shap_values_et, columns=X.columns)
...
output_path = os.path.join(OUTPUT_DIR, "ExtraTrees_Results.xlsx")
...
with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
    ...
    shap_et_df.to_excel(writer, sheet_name="SHAP_ExtraTrees", index=False)

print(f"\nAll results saved to '{output_path}'")
print("  Sheets: Actual_vs_Predicted | Actual_vs_Predicted_Train | Metrics | SHAP_ExtraTrees")
...
plot_rec_curve(y_test, y_pred_test, model_name="ExtraTrees", color="green")