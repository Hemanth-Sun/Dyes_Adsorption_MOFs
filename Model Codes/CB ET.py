...
OUTPUT_DIR = r"D:\PROJECTS\Dye Adsorption on MOFs\Codes\CB ET"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# --- Step 1: Load Dataset ---
df = pd.read_csv(os.path.join(OUTPUT_DIR, "D:\\PROJECTS\\Dye Adsorption on MOFs\\Datasets\\final MBMO.csv"), encoding='latin1')
...
for model_name, model, X_data in [('CatBoost', best_cat, X_train_scaled),
                                   ('ExtraTrees', best_et, X_train_scaled)]:
...
estimators = [('cat', best_cat), ('et', best_et)]
...
plt.title("Stacking Ensemble (ET + CB): Adsorption Capacity Prediction", fontsize=15, weight='bold')
...
shap_values_avg = (shap_values_cat + shap_values_et) / 2

# SHAP summary plot (averaged)
shap.summary_plot(shap_values_avg, X_test_scaled, feature_names=X.columns, show=False)
plt.title("SHAP Summary Plot - Stacking (ExtraTrees + CB)", fontsize=13, weight='bold')
plt.savefig(os.path.join(OUTPUT_DIR, "Stacking_ET_CB_SHAP_summary.png"), dpi=300, bbox_inches="tight")
plt.close()

# SHAP bar plot (averaged)
shap.summary_plot(shap_values_avg, X_test_scaled, feature_names=X.columns, plot_type="bar", show=False)
plt.title("SHAP Feature Importance - Stacking (ExtraTrees + CB)", fontsize=13, weight='bold')
plt.savefig(os.path.join(OUTPUT_DIR, "Stacking_ET_CB_SHAP_bar.png"), dpi=300, bbox_inches="tight")
plt.close()
...
shap_cb_df = pd.DataFrame(shap_values_cat, columns=X.columns)
...
output_path = os.path.join(OUTPUT_DIR, "Stacking_ET_CB_Results.xlsx")
...
with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
    ...
    shap_cb_df.to_excel(writer, sheet_name="SHAP_CatBoost", index=False)
    shap_et_df.to_excel(writer, sheet_name="SHAP_ExtraTrees", index=False)


print(f"\nAll results saved to '{output_path}'")
print(
    "  Sheets: Actual_vs_Predicted | Actual_vs_Predicted_Train | Metrics | SHAP_Averaged | SHAP_CatBoost | SHAP_ExtraTrees")
...
plot_rec_curve(y_test, y_pred_stack, model_name="Stacking_Ensemble_ET_CB")