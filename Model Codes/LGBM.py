...
OUTPUT_DIR = r"/users/hemanthsunkara/downloads/outs/lgbm/MBMO"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# --- Step 1: Load Dataset ---
df = pd.read_csv("/users/hemanthsunkara/downloads/ML_MBMO.csv", encoding='latin1')
...
lgbm_random = RandomizedSearchCV(
    ...
    lgbm_param_grid, n_iter=30, cv=5, scoring='r2',
...
cv_rmse = cross_val_score(best_lgbm, X_train_scaled, y_train, cv=5, scoring='neg_root_mean_squared_error')
cv_r2 = cross_val_score(best_lgbm, X_train_scaled, y_train, cv=5, scoring='r2')
print(f"\nLightGBM CV R²  : {cv_r2}")
print(f"LightGBM CV RMSE: {np.abs(cv_rmse)}")
print(f"LightGBM Mean CV RMSE: {np.mean(np.abs(cv_rmse)):.3f}")

# --- Step 12: Fit LightGBM Model ---
...
plt.title("LightGBM: Adsorption Capacity Prediction", fontsize=14, weight='bold')
...
# --- Step 16: SHAP Analysis for LightGBM ---
explainer_lgbm = shap.TreeExplainer(best_lgbm)
shap_values_lgbm = explainer_lgbm.shap_values(X_test_scaled)

# SHAP summary plot
shap.summary_plot(shap_values_lgbm, X_test_scaled, feature_names=X.columns, show=False)
plt.title("SHAP Summary Plot - LightGBM", fontsize=13, weight='bold')
plt.savefig(os.path.join(OUTPUT_DIR, "LightGBM_SHAP_summary.png"), dpi=300, bbox_inches="tight")
plt.close()

# SHAP bar plot
shap.summary_plot(shap_values_lgbm, X_test_scaled, feature_names=X.columns,
                  plot_type="bar", show=False)
plt.title("SHAP Feature Importance - LightGBM", fontsize=13, weight='bold')
plt.savefig(os.path.join(OUTPUT_DIR, "LightGBM_SHAP_bar.png"), dpi=300, bbox_inches="tight")
plt.close()
...
shap_lgbm_df = pd.DataFrame(shap_values_lgbm, columns=X.columns)
...
output_path = os.path.join(OUTPUT_DIR, "LightGBM_Results.xlsx")
...
with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
    ...
    shap_lgbm_df.to_excel(writer, sheet_name="SHAP_LightGBM", index=False)

print(f"\nAll results saved to '{output_path}'")
print("  Sheets: Actual_vs_Predicted | Actual_vs_Predicted_Train | Metrics | SHAP_LightGBM")
...
plot_rec_curve(y_test, y_pred_test, model_name="LightGBM", color="green")