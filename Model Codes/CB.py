...
# --- Step 1: Load Dataset ---
df = pd.read_csv(r"D:\PROJECTS\Dye Adsorption on MOFs\Datasets\final MBMO.csv", encoding='latin1')
...
cv_rmse = cross_val_score(best_cat, X_train_scaled, y_train, cv=5, scoring='neg_root_mean_squared_error')
cv_r2   = cross_val_score(best_cat, X_train_scaled, y_train, cv=5, scoring='r2')
print(f"\nCatBoost CV R²  : {cv_r2}")
print(f"CatBoost CV RMSE: {np.abs(cv_rmse)}")
print(f"CatBoost Mean CV RMSE: {np.mean(np.abs(cv_rmse)):.3f}")


# --- Step 12: Fit CatBoost Model ---
best_cat.fit(X_train_scaled, y_train)


# --- Step 13: Predictions ---
y_pred_test  = best_cat.predict(X_test_scaled)
y_pred_train = best_cat.predict(X_train_scaled)
...
plt.title("CatBoost: Adsorption Capacity Prediction", fontsize=14, weight='bold')
...
# --- Step 16: SHAP Analysis for CatBoost ---
explainer_cat   = shap.TreeExplainer(best_cat)
shap_values_cat = explainer_cat.shap_values(X_test_scaled)

# SHAP summary plot
shap.summary_plot(shap_values_cat, X_test_scaled, feature_names=X.columns, show=False)
plt.title("SHAP Summary Plot - CatBoost", fontsize=13, weight='bold')
plt.savefig("CatBoost_SHAP_summary.png", dpi=300, bbox_inches="tight")
plt.close()

# SHAP bar plot
shap.summary_plot(shap_values_cat, X_test_scaled, feature_names=X.columns,
                  plot_type="bar", show=False)
plt.title("SHAP Feature Importance - CatBoost", fontsize=13, weight='bold')
plt.savefig("CatBoost_SHAP_bar.png", dpi=300, bbox_inches="tight")
plt.close()
...
output_path = "CatBoost_Results.xlsx"
...
print("  Sheets: Actual_vs_Predicted | Actual_vs_Predicted_Train | Metrics | SHAP_CatBoost")
...
def plot_rec_curve(y_true, y_pred, model_name="Model", color="green"):
    ...
    plt.show()
    print(f"REC curve saved as {model_name}_REC_curve.png")


plot_rec_curve(y_test, y_pred_test, model_name="CatBoost", color="green")